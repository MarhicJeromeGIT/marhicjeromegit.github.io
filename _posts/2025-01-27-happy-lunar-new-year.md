...
date: 2025-01-27 06:28 +0000
author_profile: true
---

This week it's Lunar New Year, or Têt celebrations in Vietnam. Which mean my daughter school is off, which mean I get approximately 30 seconds of free time per day. Despite that, it was hard to miss the hype regarding the release of Deepseek R1. I didn't have time to read the paper or anything, but I gave it a quick try at [chat.deepseek.com](chat.deepseek.com). I still don't have a good handle on what is a good "reasoning question", but I came out with a good use case today: coming up with a memorable port number for a service I'm hosting locally.

> Can you come up with a good, 4 number goroawase for a port number ? I want to host a RSS reader (miniflux) so something related to reading/RSS/blogs if possible (otherwise it's ok)

(Goroawase is Japanese word-play with numbers, more on [Wikipedia](https://en.wikipedia.org/wiki/Japanese_wordplay)).
Contrary to OpenAI which hides them, it was fun seeing the model thoughts - in total the model thought for 94 seconds, here are a few excerpts of the thoughts:
...
> Alternatively, the word "blog". B is 2nd letter, L is 12th, O is 15th, G is 7th. 2-12-15-7. Doesn't fit into 4 digits. Hmm.
...
As a comparison, I asked the same prompt to ChatGPT 4o and got this:
...
Which is quite bad (it should be 4 instead of 8). o1 think for 27 seconds and ame up with:
>1106 → いい本 (ii hon)
...
They are building an elevated highway not far from my place, so I get to see the progress daily. Currently they are laying down the beams across the pillars. It's quite impressive from my point of view (we don't see a lot of those in France), must require a lot of organization. Last week I got stuck in a 10 person, 1 hour meeting to discuss a feature to allow for the import of a CSV in our app. In those time I kind of wish I was doing some "real" work instead of this.